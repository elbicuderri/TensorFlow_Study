...
class ResidualBlock(tf.keras.layers.Layer):
    def __init__(self, out_channels, kernel_size=3, padding='same', downsample=True, use_bias=False, trainable=True):
...
class SimpleResNet(tf.keras.Model):
    def __init__(self):
        ...
        self.block11 = ResidualBlock(16, 3, 'same', downsample=False, trainable=self.trainable)
        self.block12 = ResidualBlock(16, 3, 'same', downsample=False, trainable=self.trainable)
        self.block21 = ResidualBlock(32, 3, 'same', downsample=True, trainable=self.trainable)
        self.block22 = ResidualBlock(32, 3, 'same', downsample=False, trainable=self.trainable)
        self.block31 = ResidualBlock(64, 3, 'same', downsample=True, trainable=self.trainable)
        self.block32 = ResidualBlock(64, 3, 'same', downsample=False, trainable=self.trainable)
